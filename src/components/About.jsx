...
function About() {
  return (
    <section id="about" className="section-container section-spacing">
      <SectionHeading
        eyebrow="About Me"
      ...
      <div className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <motion.article
          className="glass-card min-w-0 rounded-[1.5rem] p-5 sm:rounded-[2rem] sm:p-9"
        ...
        <div className="grid min-w-0 gap-5 sm:grid-cols-2 lg:grid-cols-1">
          {highlights.map((item, index) => {
            ...
            return (
              <motion.article
                key={item.title}
                className="glass-card min-w-0 rounded-3xl p-5 transition hover:-translate-y-1 hover:border-cyan-300/40 hover:shadow-glow sm:p-6"
